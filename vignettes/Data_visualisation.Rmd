...
predicted %>%
 mutate(infectious_gp = infectious_gp / 40) %>%  # seeding rate = 40 (optional step)
 mutate(predicted_presence = if_else(infectious_gp > 0, 1, 0))
...
```{r}
wind_dat <-
  read_excel(system.file("extdata", "wind_data_for_wind_rose.xlsx",
                         package = "ascotraceR")) %>%
  mutate(
