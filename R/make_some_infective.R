...
make_some_infective <- function(daily_vals, latent_period = 200) {
  ...
  for (i_row in seq_len(NROW(newly_infectious))) {
    ...
    if (
      ...
      daily_vals[["paddock"]][
        row_index,
        susceptible_gp := paddock_vals[, susceptible_gp] - infections_new
      ]
    }

    daily_vals[["paddock"]][
      row_index,
      infectious_gp := daily_vals[["paddock"]][row_index, infectious_gp] +
        infections_new
    ]
  }
