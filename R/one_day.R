...
#'  query date `i_date`
#' @param max_gp numeric double a function of
#' `max_gp_lim x (1 - exp(-0.138629 x seeding_rate))
...
#'  model including:
#'  a `paddock` an 'x' 'y' data.table, iteration day (i_day), cumulative daily
#'  weather data, such as:
#'  cumulative degree days (cdd),
#'  cumulative wet hours (cwh) and
#'  cumulative rainfall in mm (cr);
#'  standard growing points assuming growth is not impeded by infection
#'   (`gp_standard`),
#'   new growing points produced in the last 24 hours (`new_gp`), an 'xy'
#'   `data.table` of only infectious growing points,
#'   and new_infections 'xy' data.table indicating exposed growing points in the
